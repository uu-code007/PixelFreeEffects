...
Java_com_hapi_pixelfree_PixelFree_native_1create(JNIEnv *env, jobject thiz) {
    PFPixelFree *px;
    px = PF_NewPixelFree();
    __android_log_print(ANDROID_LOG_INFO, "[PixelFree]", "native_create-----");
...
Java_com_hapi_pixelfree_PixelFree_native_1release(JNIEnv *env, jobject thiz, jlong handler) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "[PixelFree]", "native_release-----");
}
...
Java_com_hapi_pixelfree_PixelFree_native_1processWithBuffer(JNIEnv *env, jobject thiz,
    ...
    int texture = PF_processWithBuffer(px, input);
    ...
    return reinterpret_cast<jint>(texture);
}
...
Java_com_hapi_pixelfree_PixelFree_native_1auth(JNIEnv *env, jobject thiz, jlong handler,
    ...
    __android_log_print(ANDROID_LOG_INFO, "[PixelFree]", "name_str----- %s",str);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_hapi_pixelfree_PixelFree_native_1pixelFreeSetBeautyExtend(JNIEnv *env, jobject thiz,
                                                                   jlong handler, jstring value) {
    // TODO: implement native_pixelFreeSetBeautyExtend()

    auto *px = reinterpret_cast<PFPixelFree *>(handler);
    const char* str;
    jboolean isCopy;
    str = env->GetStringUTFChars(value, &isCopy);
    PF_pixelFreeSetBeautyFiterParam(px, PFBeautyFiterExtend, (void *)str);
}
extern "C"
JNIEXPORT void JNICALL
Java_com_hapi_pixelfree_PixelFree_native_1pixelFreeSetBeautyType(JNIEnv *env, jobject thiz,
                                                                 jlong handler, jint type,
                                                                 jint value) {
    // TODO: implement native_pixelFreeSetBeautyType()
    auto *px = reinterpret_cast<PFPixelFree *>(handler);
    PF_pixelFreeSetBeautyFiterParam(px,type, &value);
}