...
@class PFCamera;
typedef NS_ENUM( NSInteger, PFCameraFocusModel) {
    /* 先找人脸对焦模式 */
    PFCameraModelAutoFace,
    /* 固定点对焦模式 */
    PFCameraModelChangeless
};


@protocol PFCameraDelegate <NSObject>

- (void)didOutputVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer;

@end

@protocol PFCameraDataSource <NSObject>

- (CGPoint)faceCenterInImage:(PFCamera *)camera ;

@end


@interface PFCamera : NSObject
@property (nonatomic, weak) id<PFCameraDelegate> delegate;
@property (nonatomic, weak) id<PFCameraDataSource> dataSource;
