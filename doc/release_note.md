#### 更新日志
**2026-06-04 日更新** SDK v2.5.09

- 更新人脸检测模型
- 增加 mask 磨皮配置。默认不开启，开启后只人脸部分磨皮
- 增加在线鉴权续费
...
- **美体（身体塑形）**
  - 基于人体 **25** 关键点，新增/整理美体单项调节，与 Demo 中 `AdjustmentItem` 展示名称一致，包括：瘦身、瘦肚子、瘦腰、沙漏腰、曲线、全身瘦、提跨、丰臀、丰胸、长腿、天鹅颈、瘦肩膀、直角肩、左/右大臂、左/右小臂、左/右大腿、左/右小腿等。
  - 单项默认强度 **0.5** 为中性（无效果），建议调节范围 **0.0 ~ 1.0**；提跨、丰臀、长腿、瘦肚子等项的方向性含义与头文件 `PFBodyBeautyType` 注释一致。
  - **接口**：iOS `pixelFreeSetBodyBeautyParam:value:`；C / 原生底层 `PF_pixelFreeSetBodyBeautyParam`；`key` 为 `PFBodyBeautyType` 枚举值，`value` 为 `float *`（或各语言对应浮点参数）。
  - **文档**：已更新 `doc/api_iOS.md`、`doc/api_android.md`、`doc/api_flutter.md`、`doc/api_c.md` 美体章节；主 README / README_en 功能列表补充美体说明；UniApp README 增加美体说明与文档链接。
- **美型（v2.5.06 既有能力，2026-03-03 说明）** 新增美型细分参数（眼/鼻/眉/唇）：
  - 眼睛上下、眼高低
...
- C API 文档补充：新增美型枚举注释，包含默认值与调节含义

**2026-03-03 日更新** SDK v2.5.04
